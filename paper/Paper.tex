...
\and
\IEEEauthorblockN{Bruce MacDonald}
...
Furthermore we propose a new concept of development support %and qualitätssicherung
...
Imagine a medication reminder application which guides a person through the process of medication intake. All interaction is done by showing textual instructions on a display. An error in the program logic could cause an already taken medication to be reminded again and may injure an Alzheimer's diseased person by medication overdosage.
This example is taken from a healthcare robotics project at the University of Auckland, New Zealand. Scientists are working on applications such as the medication reminder~\cite{medicationreminder} which run on mobile robots serving in nursing homes for the elderly. Equipped with a touch display and several external devices such as a blood pressure measurement tool the robots aim to support and entertain the elderly in their daily activities~\cite{5649910}\cite{healtbot_new}.
The applications are meant to be developed by healthcare professionals using Robostudio~\cite{robostudio}, a visual programming environment for rapid authoring and customization of complex services on a personal service robot.
%state machine based processes and a graphical tool for the designing of human robot interaction.
Generally these people don't have profound knowledge of the complex mathematical syntax used for formal verification methods such as computation tree logic (CTL) or linear temporal logic (LTL). Nevertheless certain safety guarantees must somehow be delivered in order to gain users' trust and to prevent any harm or injuries by the healthbots.
...
So we need a mechanism for the specification of such safety constraints which doesn't require special or expert knowledge and thus is easy to use for all kind of programmers. A visual language intended to fill this gap is presented in section~\ref{sec:visualformalism}.
...
%Dieses beispiel ist eine Anwendung eines umfangreichen Healthcare-Projekts, an dem im robotics lab der Universität von Auckland gearbeitet wird. Die zustansbasierten Anwendungen laufen auf Cafero, einem mit blutdruckmessgerät und anderen tools austestatteten mobilen Roboter mit Touchdisplay.
%Die einzelnen Anwendungen werden von Healthcare-Professionals mit dem Tool Robostudio [cite] entwickelt, das eine graphische oberfläsche zur modellierung von zustandsmaschinen externen services und  bietet.
...
%Dennoch möchte man gegenüber den Patienten, die von solchen Robotern gepflegt werden, ein gewisses Level an Sicherheit gewährleisten können. Beispielsweise soll in obigem Beispiel sichergestellt werden können, dass ein Medikament nach der Einnahme nicht erneut in einer Erinnerung auftaucht, oder dass bei Nichteinnahme ein Mitarbeiter benachrichtigt wird.
%Dazu bedarf es allerdings Mechanismen zur spezifizierung solcher Sicherheitseigenschaften, die für den Programmentwickler zugänglich und einfach (am besten ohne spezialwissen) zu benutzen sind.
...
Even though this approach touches the subject of healthcare it barely matches our problem. HomeTL focuses rather on monitoring temporal boundaries of a patient's behavior than on ensuring functional safety of an implemented program. But it depicts how nontechnical constraint design can be realised and its graphical design gives a good example how to handle time dependencies.
...
% soll hier noch gesagt werden, dass wir davon ausgehen, dass es keine endzustände gibt?

We want to introduce a visual language for the defining of safety constraints for state machine definition of robot behaviour which is suitable for non-experts in the formal methods domain. Ideally its appliance is easy to use and intuitive to read in order to facilitate maintainability.
Hence, the graphical formalism should abstract from the usually complex and mathematical concept of conventional formal methods but still stay unreduced in expressiveness compared to LTL as far as possible.
 
Furthermore a heuristic should be developed of how to identify reasonable constraints on an underlying state machine program. 
But since the particular domain distinguishes the significance of constraints respectively it might be difficult to show a universal heuristic working for all use cases. Hence, in this paper the concept of development support by constraint generation is examined within the domain of healthcare robotics. As a special requirement to the heuristic at least the following constraints are to be found for the medication reminder application:
...
Some of the statements don't make sense of course if patients lie or pretend to take their medication. As our approach tries to validate state machine behaviour rather than human attitudes we assume correct collaboration or the use of tools which can recognize fairly medication intake. Furthermore also the correctness of external services used by the medication reminder such as the database module holding all reminders is not taken into account.
...
% Gefundene probleme, die man erwähnen könnte: beim if operator ist das übergeordnete always nicht intuitiv.

The presented visual language is made for expressing constraints for state machines. Similar to Del Bimbo's formula representation~\cite{520786}, a constraint consists of nested operators and propositions in a hierarchical manner. However we surrender the three dimensional idea and focus on two dimensional blocks and their compositions instead, what is also used by HomeTL~\cite{4341725}.
...
Regarding to the requirements the visual language should be very easy to use, especially for non-experts. Furthermore it should provide an intuitive appliance which is ideally learnable within little time. So there are some design decisions an editor for this visual language has to fulfill:

\begin{itemize}
	\item Each operator has a different significant color flavor. This kind of classification enables a faster subconscious identification by the user.
	\item All editing is simply done by only drag and drop. Operators and propositions can be moved and dropped onto other operators. A tool bar provides all kinds of operator and proposition types to be instantiated and ideally a trash can for deleting existing operators.
	...
	\item Multiple nested operators of the same kind which have no order priorities such as \emph{OR} or \emph{AND} appeared to make the look unnecessarily complex. These operators can be merged as shown in figure~\ref{fig:nested_and}.
	...
	\item There is no button for constraint creation / editing except operator and proposition providers, which let the developer create new instances of them. Having no additional buttons reduces complexity and keeps it simple.
\end{itemize}
...
In order to support the developer in specifying safety constraints, automated generation and suggestion of constraints might be helpful. But it's helpful only then, if the suggested constraints are relevant and reasonable. This however depends on the domain and use case.
Regarding to the postulated constraints in the requirements in section~\ref{sec:requirements} it turned out that all therefor relevant states have got one of the following characteristics:
...
In order to compute the required constraints and possibly even more of them automatically we manage with so called subgraphs defined as follows:
...
\begin{itemize}
	...
	\item There is an end state ($\beta$-state) in which all possible paths starting from start state come together again.
	\item All states between start and end state have no other incoming transitions than the ones originally coming from start state. That means there is no path to visit these states exept through start state.
	\item start state may have incoming transitions from states not contained in the subgraph.
	\item end state may have incoming and outgoing transitions from and to states not contained in the subgraph.
\end{itemize}
...
Using this specification all possible subgraphs can be detected within a state machine. If a subgraph contains neither inner (infinite) loops nor final states (states having no outgoing transition) we can derive the following valid LTL formulas for each subgraph (with $\alpha$ as proposition for ``$\alpha$ is current state'' and similarily for $\beta$ and all $\gamma$'s):
...
All formulas can be translated to the visual formalism presented in section~\ref{sec:visualformalism} and shown to the user. The accordant LTL formulas for the postulated constraints in section~\ref{sec:requirements} belong to number \ref{eq:second} and \ref{eq:fifth} of the subgraph based formula templates and are as follows:
...
To experience and evaluate the visual formalism as well as the usefulness of constraint generation we developed an editor which provides functionalities all proposed functionalities while taking the requirements into account.
It's implemented as a java swing component, making it easy to integrate the visual language into other java applications. An API allows to set the underlying state machine what also influences the state propositions available for constraint editing. Furthermore a particular model checker can be specified to be used for constraint validation. As default, the symbolic model checker NuSMV~\cite{springerlink:10.1007/s100090050046}~\cite{NuSMV2} is used for constraint validation.

Figure~\ref{fig:editor} shows a snapshot of the editor's environment: All functionality needed for constraint editing is provided in the tool bar on the right which contains drag providers for the creating of all operator and proposition types as well as a trash for deleting. Constraints can be composed in the dashboard in the middle of the editor by drag and drop. The tab functionality on the left allows to manage several constraints. Each tab schows a small thumbnail of the constraint and a symbol indicating its validity what is a warning shield for ``incomplete'', a green shield for ``valid'', a red cross for ``invalid'' or an animated ring for ``validation in progress''. The magic wand button within the tab pane triggers the constraint generation.
...
In order to evaluate the proposed safety functionalities on various programs the tool was integrated into Robostudio~\cite{robostudio}. It allows the specification of state machine based programs whose behaviour can then be examined by visual constraints. As an example of how to use them we will have a closer look at the simplified medication reminder application whose underlying screen-flow is explained in figure~\ref{fig:medicationreminder}. Starting from ``Menu'' the user can choose from several services such as blood pressure measurement ore entertainment mode. In the given example these services are simplified to just one state ``additional services'' since we want to focus only on the medication reminder part. In ``Polling'' the database is checked for a pending reminder, and if there is one the medication intake guidance is triggered. The patient can state if he's already taken his medication, or decide whether he wants to take it or not. In the latter he may give a reason for it and a caregiver gets notified about this incident. Otherwise it will result in ``Well done!'' after completing intake.

%we can now vergleichen die oben genannten bedingungen.
...
%Vorstellung des medication reminders
...
% TODO: hier vlt ein komponentendiagramm?

%Der Editor wurde in robostudio eingebunden, ein werkzeug zur erstellung von healthcare applicationen (elderly) basierend auf zustandsmaschinen. (bereits in der einleitung erwähnt). Über die darin definierbaren zustansmaschinen können wir nun aussagen mit der vl erstellen.

%das tool wurde den healthcare professionals vorgelegt und die ergebnisse werden gespalten in den folgenden zwei unterpunkten beschrieben (vl und constraint generierung).

%Robostudio was used as a platform to integrate the visual language described in this paper .
%basierend auf diesen healthcare applicationen können nun sicherheits constraints über unser tool definiert werden.

%rs was used as a platform to integrate the safety critical visual programming language described in this paper. fs's modular design allows the usage of the state machine model and add plugins to be integrated into the VPE, making it usabble as a plat

%personal service robot

%visual programming environment
%We embedded the editor into Robostudio~\cite{robostudio}, a tool for the creating of healthcare applications based on state machines.

%an editor for state machine based processes and a graphical tool for the designing of human robot interaction
...
Owing to the elaborated performance of NuSMV even the validation of constraints on huge and complex programs is fast and allows quick feedback. Besides, the validation itself runs in background without locking the dashboard. Therefore we got rid of waiting times and disruptions during constraint development what would be very well the case with conventional model checking where constraint development and constraint validation are alternating occupations. Thus it's considered as an improvement of user experience.


\subsection{Constraint generation}
\label{sec:prototypeandevaluation_constraintgeneration}

The automated constraint generation can be triggered by a click on the magic wand button in the tab area. After activation a dashboard gets opened in a new tab for each found constraint, and validation is initiated immediately. In case of the medication reminder application six constraints are found, whereof \emph{A)} and \emph{B)} shown in figure~\ref{fig:generatedconstraints} are actually matching the postulations in the requirements in section~\ref{sec:requirements}.
The constraint used for demonstration in the previous chapter gets also generated, however \emph{B)} forms an intensified restriction of it.

Constraint \emph{A)} ensures the ``Polling'' state to be always eventually visited again. If a pending medication is not already taken, constraint \emph{B)} guarantees intake or staff notification before next reminders can be read from database.

We showed that the subgraph approach is working for the medication reminder application, and there was even one more reasonable constraint found by the heuristic: Constraint \emph{C)} ensures that during the medication intake process the program can not switch back to the menu or other services such as entertainment mode ore blood pressure measurement.
...
%Da der subgraph-finde-algorithmus als brute force algorithmus implementiert wurde kann bei großen programmen (ab ca. 1000 Zuständen und sehr vielen Verzweigungen) der findeprozess teils sehr lange (mehrere Minuten) dauern. Um den warteprozess erträglicher zu machen wird während der findung ein dialog mit dem aktuellen fortschritt und einem abbrechen button angezeigt.
...
%	\item constraints bewerten (nach gewichtung?) und nur wichtige vorschlagen? Oder zumindest sortieren
% \item es wird nur gutes gefunden (also was passieren muss). es werden keine constraints gefunden, die schlechte abläufe darstellenund verbieten
...
In this paper we represented a visual formalism for the defining of safety constraints and a concept of automated constraint generation. Both features are implemented and assembled in an editor for standalone or integration use. The visual language is pretty simple what makes it accessible to a wider range of software developers. Nevertheless its expressiveness is fairly powerful and allows serious model checking.

The concept of constraint generation worked very well for our use case the medication reminder application and all postulated constraints were found. Since we even discovered a new reasonable constraint we are convinced this concept as well as the visual language for constraint defining can be a beneficial supporting thing for plenty of developers.
