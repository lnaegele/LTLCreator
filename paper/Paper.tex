...
\and
\IEEEauthorblockN{Andreas Angerer}
\IEEEauthorblockA{Department of Software Engineering\\ 
University of Augsburg\\
86135 Augsburg, Germany\\
...
\begin{abstract}
%\boldmath
Nowadays robots are no longer confined to only industrial use but increasingly assigned to jobs within human workspaces. Even the programming of robots is shifting from experienced software developers to the consumers themselves who need to customize the robot's behavior in order to match individual requirements.
...
%----------------------------------------------------------------------
% SECTION II: Requirements (renamed this to 'Goals' to perhaps better reflect the content)
%----------------------------------------------------------------------
\section{Goals}
...
\begin{itemize}
	...
	\item Multiple nested operators of the same kind which have no order priorities such as \emph{OR} or \emph{AND} appear unnecessarily complex. These operators can be merged.
\end{itemize}
...
%\begin{figure}[htbp]
  %\centering
  %\includegraphics[scale=0.4]{and_simplify} %width=0.7\linewidth
