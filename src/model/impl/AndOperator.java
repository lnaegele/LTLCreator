...
public class AndOperator extends AbstractOperator {
	...
	public AndOperator(AbstractOperator op1, AbstractOperator op2) {
		this();
	...
	private static boolean needBrackets(AbstractOperator op) {
		...
		return !((op instanceof StateOperator) || (op instanceof AndOperator) || (op instanceof NotOperator));
	}
	...
	public Dimension getOperatorSize() {
		...
		return new Dimension(left+right, insets.top + a_ps.height + vertical_space_between_buckets + b_ps.height + insets.bottom);
	}
	...
	protected void layoutBuckets() {
		...
		this.a.setBounds(centerLineX - a_cl, insets.top, a_ps.width, a_ps.height);
		this.b.setBounds(centerLineX - b_cl, insets.top + a_ps.height + vertical_space_between_buckets, b_ps.width, b_ps.height);
	}
	...
	public void paintOperator(Graphics2D g, boolean considerHighlighting) {
	    ...
	    int textCenterPosY = this.getInsets().top + this.a.getPreferredSize().height + (vertical_space_between_buckets / 2);
		g.setFont(this.font);
		Utils.paintTextCentered(g, "AND", this.getCenterLineX(), textCenterPosY);
	}
