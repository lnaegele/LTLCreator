...
%\end{wrapfigure}
The two newest robots IrobiQ and Cafero are shown in figure~\ref{fig:robots}.
\begin{figure}[htbp]
  \centering
  ...
  \label{fig:robots}
\end{figure}
These mobile robots have got a touch display for presenting screens and receiving user input. Furthermore they are equipped with additional external tools such as the blood pressure measurement device.
...
The interpreter processes programs written in Robot Behavior Description Language (RBDL), a particular DSL based on XML, which has been developed specially for the healthcare robotic applications. It allows defining state machine based program behavior and creating UI elements for screen visualizations. An example implementation of one state in RBDL is shown in listing~\ref{lst:RBDL}.
...
%\lstset{language=XML}
\begin{lstlisting}[language=XML, captionpos=b, breaklines=true, showspaces=false, showtabs=false, tabsize=2, caption=Example of a state defined by RBDL~\cite{robostudio}., label=lst:RBDL]
<state no="84">
	<backgroundactions>
...
For each state of the program, backgroundactions can be defined. They are are transparent to the user and run in background. Text messages can be sent or external devices can be acessed, for example. The latter is realized with web services to obtain a modular design.
...
For this work the most important thing about the RBLD to remember is, that it can define state machine behavior with states, transitions and events. This fact was basement for later decisions regarding used safety mechanisms.
