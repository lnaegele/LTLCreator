...
The \emph{Background Actions Editor Window} (5) lets the developer define background actions, which are executed transparently such as accessing external devices or sending messages.
...
All influences of the environment which cause a change to the system are called \emph{events}. They trigger transitions which transfer the system into a new (current) state. The fact that a state machine reacts to events is the reason why state machines are also known as \emph{reactive systems}.
...
Also the explanation of state machines above is no formal definition; it can be found in literature~\cite{statemachines, statemachinesandformalmethods}.
...
\begin{description}
	...
	\item[$\varphi \mathcal{R} \psi$] It expresses that $\psi$ is true at least as long as there is no occurrence of a state where $\varphi$ is true. Its meaning is $\varphi$ ``RELEASES'' $\psi$.
